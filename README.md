...
优先考虑决策少的元素（重量大的先来，它选择有限）

ans从n开始，实时记录下最少的车的情况k（所以能放一起的优先先选择放一起），之后如果有超过k的就直接剪去了

注意这个写法，ans是全局的，随时用来剪枝
...
注意这里不是一根一根按顺序处理小木棍，而是一根一根完成长木棍处理，一根完整填好之后再填另一根（并且让小木棍下标都是递增的，135和531不用重复搜），这样方便剪枝。注意一些剪枝策略。



### dfs迭代加深-加成序列

限制depth大小，比起之前用>= ans就return，改成>=depth就return

但是depth是无法确定的，所以在调用时从1开始慢慢往上走：
